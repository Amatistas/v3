...
class Insert2Tables
{
    ...
    function compraActualAlmacen($data)
    {
        // query to insert record
        /*  $query = "CALL compra_actual_almacen($cd_pro_id,$alm_id,$cd_can,$pst_id);";
         */ // prepare query
        $stmt = $this->conn->prepare($query);
        // execute query
        if ($stmt->execute()) {
    ...
    function notaCreditoAlmacen($data)
    {
        $com_id  = $data->com_id;
        $id_producto = $data->id_pro;
        $cantidad = $data->valor_nota_credito;
        $id_almacen = $data->id_almacen;

        // query to insert record
        $query = "CALL notacreditoproducto($id_producto,$com_id,$cantidad,$id_almacen)";
        /// prepare query
        $stmt = $this->conn->prepare($query);
        // execute query
        if ($stmt->execute()) {
    ...
    function movimientoAlmacen($idnotacredito, $data, $info)
    {
        
        $id_nc = $idnotacredito['com_id'];
        $fecha = $info->com_fecdoc;
        $cantidad = $data->valor_nota_credito;
        $id_almacen = $data->id_almacen;
    
        // query to insert record
         $query = "CALL movimiento_almacen('$fecha',$id_nc,0,0,$cantidad,$id_almacen)"; 
         /// prepare query */
        $stmt = $this->conn->prepare($query);
        // execute query
        if ($stmt->execute()) {
